...
function NavCard({
  ...
  if (!article) {
    return (
      <div className="rounded-xl border border-dashed border-[#dcc8b3] bg-surface-card p-4 text-sm text-ink-subtle">
        {label}: bulunamadı
      </div>
    );
  }
  ...
  return (
    <Link href={`/articles/${article.slug}`} className="rounded-xl border border-[#e2d3c1] bg-surface-card p-4 hover:border-accent">
